...
# 建立缓存目录
if CACHE and not os.path.isdir(CACHE_DIR):
    os.mkdir(CACHE_DIR)
...
def get_sannma(soup):
    ...
    for aura in all_aura:
        ...
        aura_stat = get_text(aura_weapon.find(
            'span', class_='saurytxt').contents[-1])
        sannmas[w_id] = aura_stat
    return sannmas


async def get_weaponlist():
    '''
    解析首页得到全部装备的编号
    '''
    res = None
    ...
    with timeout.timeout(40):
        res = await SESSION.get(SITEROOT, proxy=PROXY)
        content = await res.text()
    content_soup = BeautifulSoup(content, 'lxml')
    sannma_stats = None
    if SANNMA:
        sannma_stats = get_sannma(content_soup)
...
def get_remodel(resource, supports):
    '''
    获取不同改修方式的详情
    '''
    remodel_info = dict()
    ...
    for support_ship in support_ships:
        ...
        for name in support_ship:
            if isinstance(name, element.NavigableString):
                support_ship_name += get_text(name)
    ...
    for cost_line in resource.table.children:
        ...
        thead = cost_line.th
        if thead:
            ttitle = get_text(thead)
            cost = dict()
            if ttitle and ttitle != '改修必要資材' and ttitle != '★':
                tds = cost_line.find_all('td')
                buildkit_num = get_text(tds[0])
                remodelkit_num = get_text(tds[1])
                equipkit = list()
...
async def get_detail(weaponid):
    ...
    if CACHE:
        cache_path = os.path.abspath(
            '{0}/{1}.json'.format(CACHE_DIR, weaponid))
        with timeout.timeout(40):
            hand_shake = await SESSION.head(requesturl, proxy=PROXY)
    ...
    # 处理没有缓存或者缓存过期的文件
    res = await SESSION.get(requesturl, proxy=PROXY)
    content = await res.text()
    content_soup = BeautifulSoup(content, 'lxml')
    detail = dict()
    ...
    item_no = get_text(name_selector.find('span', class_='no'))
    item_title = get_text(name_selector.find('span', class_='title'))
    ...
    # 获取介绍
    item_intro = get_text(content_soup.select_one('div.article.jpdetail'))
    if item_intro:
    ...
    for status_line in status_lines:
        ...
        if status_title and status_title.has_attr('class') \
            ...
            if len(class_list) > 1:
                ...
                status_title_name = get_text(status_title)
    ...
    for extra_equip_line in extra_equip_lines:
        ...
        for extra_equip_row in extra_equip_line_children:
            ...
            ship_type = get_text(extra_equip_row)
            if not ship_type:
                continue
            if ship_type in ship_types:
                if is_ok:
    ...
    for fitting_line in fitting_lines:
        ...
        for fitting_row in fitting_line_children:
            if not fitting_row.contents:
                continue
            ship_correct = get_text(fitting_row.find('span', class_='cor'))
            ship_class = get_text(fitting_row.contents[1])
        ...
        for remodel_line in remodel_selector:
            ...
            for remodel_row in remodel_line.children:
                ...
                if remodel_row_name == 'th':
                    remodel_title = get_text(remodel_row)
                    if remodel_title == '砲台':
                        remodel_title += '特効倍率'
    ...
    support_tables = content_soup.find_all('div', class_='support-ship-table')
    resource_list = list()
    support_list = list()
    for resource_table in resource_tables:
        thead = get_text(resource_table.table.tr.th)
        if thead == '改修必要資材':
            resource_list.append(resource_table)
    for support_table in support_tables:
        thead = get_text(support_table.table.tr.th)
        if thead == '二番艦':
    ...
    for i in range(remodel_size):
        remodel_info.append(get_remodel(resource_list[i], support_list[i]))
    ...
    if build_selector:
        ...
        if secretary_selector:
            secretary = get_text(secretary_selector)
            buid['secretary'] = secretary
        buid['cost'] = dict()
        if fuel_selector:
            fuel = get_text(fuel_selector)
            buid['cost']['fuel'] = int(fuel)
        if ammo_selector:
            ammo = get_text(ammo_selector)
            buid['cost']['ammo'] = int(ammo)
        if steel_selector:
            steel = get_text(steel_selector)
            buid['cost']['steel'] = int(steel)
        if bauxite_selector:
            bauxite = get_text(bauxite_selector)
...
async def main():
    ...
    id_list, sannma_stats = await get_weaponlist()

    if SANNMA:
        akashi_json['sannma_stats'] = sannma_stats

    print('获取ID成功，开始解析. 共{}条数据.'.format(len(id_list)))
    tasks = []
    for weapon_id in id_list:
        tasks.append(asyncio.ensure_future(get_detail(weapon_id)))
    weapon_list = {}
    ...
    type_sortd_json = {}
    with open(OUTPUT, 'w', encoding='utf_8') as fjson:
...
if __name__ == '__main__':
    ...
    END = time.time()
    DELTA = END - START
    print('\n已全部完成，共用时：{0}s.'.format(round(DELTA, 3)))
    SESSION.close()
